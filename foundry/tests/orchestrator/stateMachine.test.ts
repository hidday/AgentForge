...
describe("stateMachine - invalid transitions", () => {
  it("throws StateTransitionError for a state with no outgoing transitions at all", () => {
  ...
  it("throws StateTransitionError for a valid state with an event not registered for it", () => {
    // RunState.Todo has outgoing transitions, but not for PLAN_CREATED.
    expect(() => transition(RunState.Todo, RunEvent.PLAN_CREATED)).toThrow(
      StateTransitionError,
    );
    try {
