...
describe("stateMachine - invalid transitions", () => {
  it("throws StateTransitionError when the current state has no entries in the table at all (e.g. Done is terminal)", () => {
  ...
  it("throws StateTransitionError when the state exists in the table but the event is not a valid transition for it", () => {
    // Todo has entries (RUN_REQUESTED, BLOCKED, NEEDS_HUMAN_CLARIFICATION) but not PLAN_APPROVED.
    try {
