...
function makePlanReview(): PlanReview {
  return { reviewId: "plan-rev-001", summary: "Looks fine", findings: [], overallVerdict: "approved" };
}
...
describe("PlanReviserAgent.run() operator note", () => {
  it("includes the operator note section and logs hasOperatorNote:true when provided", async () => {
    ...
    await agent.run(makePlan(), makePlanReview(), makeTaskBundle(), "run-1", {
      operatorNote: "Keep the migration reversible.",
    });

    const prompt = getPrompt();
    expect(prompt).toContain("## Operator Note");
    expect(prompt).toContain("Keep the migration reversible.");
