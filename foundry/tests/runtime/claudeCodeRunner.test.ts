...
describe("ClaudeCodeRunner.run() — arg building and envelope unwrapping", () => {
  ...
  it("falls back to scanning NDJSON lines from the end for a type:result entry", async () => {
    const ndjson = [
      "not json at all",
      "",
  ...
  it("picks the LAST type:result line when the NDJSON stream contains more than one", async () => {
    const firstResult = JSON.stringify({ type: "result", result: "stale intermediate result" });
    const lastResult = JSON.stringify({ type: "result", result: validStructuredOutput });
    const ndjson = [firstResult, lastResult].join("\n");
  ...
  it("builds a process context keyed by runtime 'claude-code' when input.runId is set", async () => {
    const processRunner = makeMockProcessRunner({
      stdout: JSON.stringify({ type: "result", result: validStructuredOutput }),
      stderr: "",
      exitCode: 0,
  ...
  it("falls back to the raw output with isApiError=false when nothing parses as JSON at all", async () => {
...
describe("ClaudeCodeRunner.chatRun() — arg filtering and error surfacing", () => {
  ...
  it("omits the process context in chatRun when input.runId is not set", async () => {
    const processRunner = makeMockProcessRunner({
      stdout: successEnvelope,
      stderr: "",
      exitCode: 0,
  ...
  it("builds a process context in chatRun when input.runId is set", async () => {
    const processRunner = makeMockProcessRunner({
      stdout: successEnvelope,
      stderr: "",
      exitCode: 0,
    ...
    await runner.chatRun(
      { prompt: "Hello", workingDirectory: "/tmp", timeoutMs: 1000, runId: "run-chat-1" },
      "chat",
    );
    ...
    expect(context).toEqual({ runId: "run-chat-1", stage: "chat", runtime: "claude-code" });
  });
