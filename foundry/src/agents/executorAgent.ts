...
export class ExecutorAgent {
  ...
  async run(
    ...
    this.logger.info({ runId, planVersion: plan.planVersion }, "Starting executor agent");
    ...
    const branchName = taskBundle.repo.workingBranch;
    await this.githubClient.createBranch(taskBundle.repo.name, branchName);

    const prNumber = await this.githubClient.createDraftPR(
