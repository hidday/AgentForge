...
describe("api client", () => {
  ...
  describe("request() success path", () => {
    ...
    it("appends a query string when a state filter is provided", async () => {
      (globalThis.fetch as ReturnType<typeof vi.fn>).mockResolvedValue(jsonResponse({ runs: [] }));
  ...
  describe("endpoint coverage", () => {
    ...
    it("getRun requests /runs/:id", async () => {
      ...
      expect(globalThis.fetch).toHaveBeenCalledWith("/api/runs/run-1", expect.anything());
    });

    it("getRunSkills requests /runs/:id/skills", async () => {
      await api.getRunSkills("run-1");
      expect(globalThis.fetch).toHaveBeenCalledWith("/api/runs/run-1/skills", expect.anything());
    });
    ...
    it("getEvents requests /runs/:id/events", async () => {
      ...
      expect(globalThis.fetch).toHaveBeenCalledWith("/api/runs/run-1/events", expect.anything());
    });
