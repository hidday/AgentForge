...
describe("OpenQuestionsPanel", () => {
  ...
  it("excludes an unanswered optional question from the submit payload while including the answered required one", async () => {
    mockApi.answerQuestions.mockResolvedValue({ ok: true, run: {} });
    ...
    // Only fill the required question; leave the optional one blank.
    const textareas = screen.getAllByRole("textbox") as HTMLTextAreaElement[];
    await userEvent.type(textareas[0], "Prod target");
  ...
  it("shows the generic fallback error message when submission rejects with a non-Error value", async () => {
