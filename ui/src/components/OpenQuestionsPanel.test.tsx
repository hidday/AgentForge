...
describe("OpenQuestionsPanel", () => {
  ...
  it("excludes unanswered optional questions from the payload on submit", async () => {
    mockApi.answerQuestions.mockResolvedValue({ ok: true, run: {} });
    ...
    // Fill only the required question; leave optionalQuestion untouched (undefined answer)
    const textareas = screen.getAllByRole("textbox") as HTMLTextAreaElement[];
    await userEvent.type(textareas[0], "Prod target");
  ...
  it("shows a generic error message when a non-Error value is thrown on submit", async () => {
